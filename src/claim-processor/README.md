...
If you have created this api with docker support (`hypermea api create claim-processor --add-docker`) then to launch the service with docker-compose:

`docker compose up -d`

If you have created this api with serverless support (`hypermea api create claim-processor --add-serverless`) then launch with:
...
| ------------------------- | ------------------------------------------------------------ | ----------------------------------------------------------- |
| HY_API_NAME               | The name of your API.  Appears in logs and emails.           | The name you used with `hypermea api create` (i.e. claim-processor)                              |
...
| HY_MONGO_DBNAME           |                                                              | The name you used with `hypermea api create` (i.e. claim-processor)                             |
| HY_API_PORT               |                                                              | 2112                                                        |
...
If using auth (e.g. `hypermea api create claim-processor --add-auth` )
...
| AUTH_REALM             | Appears in the `WWW-Authenticate` header in unauthorized requests. | claim-processor.pointw.com                       |
| AUTH_JWT_DOMAIN        |                                                              | claim-processor.us.auth0.com                     |
| AUTH_JWT_AUDIENCE      | This is the identifier a client uses when requesting a token from the auth provider.  It is a URI only (identifier only), not an actual URL (i.e. no requests are made to it) | https://pointw.com/claim-processor               |
| AUTH0_API_AUDIENCE     | When claim-processor requests a token to use the Auth0 API, this is the audience for the token. | https://claim-processor.us.auth0.com/api/v2/     |
| AUTH0_API_BASE_URL     | The base of the Auth0 API                                    | https://claim-processor.us.auth0.com/api/v2      |
| AUTH0_CLAIMS_NAMESPACE | If you configure Auth0 to insert additional claims, use this value as a namespace (prefix). | https://pointw.com/claim-processor               |
| AUTH0_TOKEN_ENDPOINT   | When claim-processor needs to call the Auth0 API, it uses this endpoint to request a token. | https://claim-processor.us.auth0.com/oauth/token |
| AUTH0_CLIENT_ID        | When claim-processor needs to call the Auth0 API, it uses this client id/secret to authenticate.  These are not the client id/secret of your application. | --your-client-id--                               |
